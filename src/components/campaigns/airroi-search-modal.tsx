...
export function AirROISearchModal({ open, onOpenChange, campaignId, onImported }: AirROISearchModalProps) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-hidden flex flex-col bg-[#13141c] border-[#363a4f]">
        <DialogHeader>
          <DialogTitle className="text-[#f0f0f6]">Search AirROI Listings</DialogTitle>
        ...
        <TabsList className="bg-[#1c1d2b] border border-[#363a4f] w-full">
        ...
        <div className="relative" ref={dropdownRef}>
          <Label className="text-[#c4c5d6] text-xs mb-1 block">Market</Label>
          <div className="relative">
            <MapPin className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-[#9395a8]" />
            <Input
          ...
          {showDropdown && (
            <div className="absolute z-50 w-full mt-1 bg-[#1c1d2b] border border-[#363a4f] rounded-lg shadow-xl overflow-hidden">
              {markets.slice(0, 8).map((market, i) => (
                <button
                  key={i}
                  onClick={() => selectMarket(market)}
                  className="w-full text-left px-4 py-2.5 hover:bg-[#363a4f] transition-colors flex items-center justify-between"
                >
                  <div>
        ...
        <TabsContent value="lookup" className="mt-3 space-y-3">
          <div>
            <Label className="text-[#c4c5d6] text-xs mb-1 block">Airbnb URL, Listing ID, or Address</Label>
            <div className="relative">
              <Hash className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-[#9395a8]" />
              <Input
          ...
          <div className="flex-1 overflow-hidden flex flex-col min-h-0">
            <div className="flex items-center justify-between py-2">
              ...
              <div className="flex gap-2">
                <Button variant="ghost" size="sm" onClick={() => setSelected(new Set(results.map(l => l.listing_id)))} className="text-xs text-[#c4c5d6]">All</Button>
                <Button variant="ghost" size="sm" onClick={() => setSelected(new Set())} className="text-xs text-[#c4c5d6]">None</Button>
                <Button
                  size="sm"
              ...
              {results.map(listing => {
                ...
                return (
                  <div key={listing.listing_id} className={`rounded-lg border transition-colors ${selected.has(listing.listing_id) ? 'border-[#6366F1] bg-[#6366F1]/10' : 'border-[#363a4f] bg-[#1c1d2b]'}`}>
                    ...
                    <div className="p-3 cursor-pointer hover:bg-white/[0.02]" onClick={() => toggleSelect(listing.listing_id)}>
                      <div className="flex items-start justify-between gap-3">
                        ...
                        <div className="flex flex-col items-end gap-1 shrink-0 text-xs">
                          {listing.avg_rating && <div className="flex items-center gap-1 text-[#F59E0B]"><Star className="h-3 w-3" /><span>{listing.avg_rating.toFixed(1)}</span><span className="text-[#9395a8]">({listing.total_reviews})</span></div>}
                    ...
                    {isExpanded && (
                      <div className="border-t border-[#363a4f] px-3 py-3 space-y-3 text-xs">
                        {/* Score breakdown */}
                        <div>
                          <p className="text-[#c4c5d6] font-medium mb-2">📊 How this score was calculated</p>
                          <div className="grid grid-cols-2 gap-2">
                            {[
                              ...
                              <div key={item.label} className="bg-[#09090f] rounded p-2.5">
                                <div className="flex items-center justify-between mb-1">
                                  <span className="text-[#c4c5d6] font-medium">{item.label}</span>
                                ...
                                <p className="text-[#c4c5d6] leading-relaxed">{item.desc}</p>
                              </div>
                            ))}
                          </div>
                        </div>
                        ...
                        {(listing.opportunity_notes || listing.outreach_angle) ? (
                          <div className="space-y-2">
                            {listing.opportunity_notes && (
                              <div className="bg-[#09090f] rounded p-2.5">
                          ...
                          <p className="text-[#9395a8] text-center italic py-1">AI analysis available for listings with score ≥ 40 when OpenAI is configured.</p>
                        )}
                      </div>
                    )}
                  </div>
